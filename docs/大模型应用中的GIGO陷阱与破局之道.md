...
近期在尝试开发开发财报点评服务时，最初采用了最"直觉"的方案：
...
- 🔢 **数据提取混乱**
- 📊 **指标计算错误**
- 📉 **分析质量低下**：基于错误数据的分析毫无价值
- ❌ **可靠性无法保证**：同样的输入，每次输出都不同
...
| 特性                 | 要求              | LLM 的能力     | 匹配度        |
| -------------------- | ----------------- | -------------- | ------------- |
| **数值精确性** | 小数点后两位准确  | 概率性文本生成 | ❌ 不匹配     |
| **逻辑一致性** | 严格的计算公式    | 上下文理解     | ❌ 不匹配     |
| **结构化提取** | 从PDF提取准确数据 | 文本模式识别   | ⚠️ 部分匹配 |
| **语义分析**   | 理解业务含义      | 强项           | ✅ 匹配       |
| **洞察生成**   | 发现趋势和异常    | 强项           | ✅ 匹配       |

**核心矛盾**：财报分析需要"精确计算"和"语义理解"的结合，而我们却把所有任务都交给了 LLM。
...
本文思考基于实际的**财报点评系统**项目实践，代码完善中，完整代码见：https://github.com/damaohongtu/fin-report-reviewer
